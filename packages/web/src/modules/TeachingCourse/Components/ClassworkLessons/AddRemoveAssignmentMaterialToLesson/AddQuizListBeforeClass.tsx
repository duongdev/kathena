...
export type AddQuizListBeforeClassProps = {
  open: boolean
  onClose: () => void
  idCourse: string
  lesson: Pick<
...
const AddQuizListBeforeClass: FC<AddQuizListBeforeClassProps> = (props) => {
  const { open, onClose, lesson, idCourse } = props
  const classes = useStyles(props)
  const [attachments, setAttachments] = useState<string[]>([])

  const [quizListBeforeClass, setClassworkAssignmentListBeforeClass] = useState<
    string[]
  >(lesson.quizListBeforeClass ?? [])
  ...
  useEffect(() => {
    setClassworkAssignmentListBeforeClass(lesson.quizListBeforeClass ?? [])
  }, [open, lesson?.quizListBeforeClass])

  const toggleClick = (id: string) => {
    const arr = [...quizListBeforeClass]
          ...
          {attachments.length
            ? attachments.map((item) => {
                const inTheListAssignment =
                  quizListBeforeClass.findIndex((i) => i === item) > -1
                return (
                  <div
...
export default AddQuizListBeforeClass
