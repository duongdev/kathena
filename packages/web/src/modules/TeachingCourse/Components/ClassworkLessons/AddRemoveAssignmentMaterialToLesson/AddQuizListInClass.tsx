...
export type AddQuizListInClassProps = {
  open: boolean
  onClose: () => void
  idCourse: string
  lesson: Pick<
...
const AddQuizListInClass: FC<AddQuizListInClassProps> = (props) => {
  const { open, onClose, lesson, idCourse } = props
  const classes = useStyles(props)
  const [attachments, setAttachments] = useState<string[]>([])

  const [quizListInClass, setClassworkAssignmentListBeforeClass] = useState<
    string[]
  >(lesson.quizListInClass ?? [])
  ...
  useEffect(() => {
    setClassworkAssignmentListBeforeClass(lesson.quizListInClass ?? [])
  }, [open, lesson?.quizListInClass])

  const toggleClick = (id: string) => {
    const arr = [...quizListInClass]
          ...
          {attachments.length
            ? attachments.map((item) => {
                const inTheListAssignment =
                  quizListInClass.findIndex((i) => i === item) > -1
                return (
                  <div
...
export default AddQuizListInClass
