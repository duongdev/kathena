...
import { Button, ImagesUploadInput, InputField, renderApolloError } from '@kathena/ui'
import InputFieldLabel from '@kathena/ui/InputField/InputFieldLabel'
import { ClassworkAssignmentDetailDocument, useAddVideoToClassworkAssignmentMutation } from 'graphql/generated'
...
const AddVideoToClassworkAssignment: FC<AddVideoToClassworkAssignmentProps> =
  (props) => {
    ...
    const addVideo = async () => {
      if (iframe.startsWith('<iframe') && iframe.endsWith('></iframe>')) {
        try {
          const dataCreated = (
            await addVideoToClassworkAssignment({
              variables: {
          ...
          const classworkAssignment = dataCreated?.addVideoToClassworkAssignment
    ...
    return (
        <div className={classes.root}>
          <Grid container spacing={5}>
            ...
            <Grid item xs={6}>
              <Stack style={{ height: '100%' }}>
                ...
                <ImagesUploadInput accept={['image/png', 'image/jpeg']} maxFiles={1} onChange={(files) => setThumbnail(files[0] ?? null)} />
              </Stack>
            </Grid>
          </Grid>
          <div
          ...
          >
            <Button onClick={addVideo} disabled={title === '' || iframe === '' || !thumbnail} style={{ width: '100px' }}>
              Thêm video
            </Button>
            <Button
              style={{ width: '100px' }}
...
export default AddVideoToClassworkAssignment
